...
from mynet import resnet18,resnet50
...
def main(flag, input_root, output_root,resnetno, end_epoch, download):
    ''' main function
    ...
    stepops = []
    for opstep in range(5):
        lr += opstep*0.001
        optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9)
        #optimizer = optim.Adamax(model.parameters(), lr=lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
        stepops.append(optimizer)

    aveloss = []
    epochtime = []
    for epoch in trange(start_epoch, end_epoch):
        if epoch <= 4:
            optimizer = stepops[epoch]
        else:
            optimizer = stepops[4]
        time_start = time.time()
        aveloss.append(train(model, optimizer, criterion, train_loader, device, task))
        val(model, val_loader, device, val_auc_list, task, dir_path, epoch)
        time_end = time.time()
        #print(aveloss)
        epochtime.append(str(time_end - time_start))


    print(aveloss)
    with open("./output/epochtime.txt", "w") as f:
        for eachtime in epochtime:
            f.write(eachtime+ '\n')
    with open("./output/aveloss.txt", "w") as f:
        for aloss in aveloss:
            f.write(aloss+'\n')
...
def train(model, optimizer, criterion, train_loader, device, task):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        theloss.append(loss.data.item())
    aveloss = np.mean(theloss)
    with open("./output/singleloss.txt", "a+") as f:
        for aloss in theloss:
            f.write(str(aloss)+'\n')
        f.write('\n')
    return str(aveloss)
